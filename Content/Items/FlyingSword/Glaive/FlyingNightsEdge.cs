...
{
    class FlyingNightsEdge : ModItem
    {
        private readonly Texture2D texture = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingNightsEdge").Value;
        private readonly Texture2D texture_ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingNightsEdge_").Value;
        public override void SetStaticDefaults()
        {
            ...
            //微光转换
            ItemID.Sets.ShimmerTransformToItem[Type] = ModContent.ItemType<RemoveLightStone>();
        }
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<FlyingNightsEdgeProj>();
            Item.shootSpeed = 16f;
            Item.buffType = ModContent.BuffType<BuffsFlyingNightsEdge>();
        ...
        {
            player.AddBuff(ModContent.BuffType<BuffsFlyingNightsEdge>(), 3600);
        ...
        {
            // 创建一个新的配方组
            RecipeGroup group = new RecipeGroup(() => Language.ActiveCulture.Name == "zh-Hans" ? 
            "【血腥屠刀·御】或【魔光剑·御】":"[Flying Blood Butcherer] or [Flying Lights Bane]",
                ModContent.ItemType<FlyingBloodButcherer>(),
                ModContent.ItemType<FlyingLightsBane>());
            // 注册配方组
            RecipeGroup.RegisterGroup("SwordMastery:FlyingNightsEdge", group);

            CreateRecipe()
               .AddRecipeGroup("SwordMastery:FlyingNightsEdge", 1) // 血腥屠刀·御或魔光剑·御
               .AddIngredient(ModContent.ItemType<FlyingMuramasa>(), 1) // 村正·御
               .AddIngredient(ModContent.ItemType<FlyingBladeofGrass>(), 1) // 草剑·御
               .AddIngredient(ModContent.ItemType<FlyingFieryGreatsword>(), 1) // 火山·御
               .AddTile(TileID.DemonAltar) // 恶魔祭坛
               .Register();
        }
    }
    public class FlyingNightsEdgeProj : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingNightsEdge";
        ...
        public override bool PreAI()
        {
            ...
            {
                if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingNightsEdge>()))
                {
                    Projectile.timeLeft = 2;
                }
                if (!player.HasBuff(ModContent.BuffType<BuffsFlyingNightsEdge>())
            ...
            idleSpot += vector + new Vector2(8, 5);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            int projType = ModContent.ProjectileType<FlyingNightsEdgeProj_>();
            ...
            if (target.CanBeChasedBy(Projectile.owner))
            {
                ...
                if (proj.ModProjectile is FlyingNightsEdgeProj_ customProj)
                    customProj.Initialize(2f, 30f, 1f);
                ...
                if (proj.ModProjectile is FlyingNightsEdgeProj_ customProj_)
                    customProj_.Initialize(-2f, 30f, 1f);
                clown = 30;
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            //c.A = 0;
            // 使用自定义颜色
            Color LightsColor = new Color(216, 71, 238);
            var value = (float)(Math.Cos(Main.timeForVisualEffects * 0.04 + Projectile.ai[2] * 0.7) * 0.3f + 0.4f);
    ...
    public class FlyingNightsEdgeProj_ : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                Dust dust = Dust.NewDustPerfect(dustPosition, DustID.Clentaminator_Purple, dustVelocity, 100, Color.DarkBlue * Projectile.Opacity, 1.2f * Projectile.Opacity);
                dust.scale *= 1.2f;
                dust.noGravity = true;
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color backDarkColor = Color.DarkSlateBlue;
            Color middleMediumColor = Color.BlueViolet;
            Color frontLightColor = Color.BlueViolet;
            ...
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.Black * 0.6f * lerpTime, Projectile.rotation + direction * 0.01f, origin, scale, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.Black * 0.5f * lerpTime, Projectile.rotation + direction * -0.05f, origin, scale * 0.8f, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.Black * 0.4f * lerpTime, Projectile.rotation + direction * -0.2f, origin, scale * 0.6f, spriteEffects, 0f);
    ...
    class BuffsFlyingNightsEdge : ModBuff
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<FlyingNightsEdgeProj>()] > 0)//检测玩家持有的弹幕数量
            {
