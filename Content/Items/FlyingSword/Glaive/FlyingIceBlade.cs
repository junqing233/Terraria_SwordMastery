...
{
    class FlyingIceBlade : ModItem
    {
        private readonly Texture2D texture = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingIceBlade").Value;
        private readonly Texture2D texture_ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingIceBlade_").Value;
        ...
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<FlyingIceBladeProj>();
            Item.shootSpeed = 16f;
            Item.buffType = ModContent.BuffType<BuffsFlyingIceBlade>();
        ...
        public override void PostUpdate()
        {
            ...
            //•	G: 238 / 255 ≈ 0.894
            //•	B: 238 / 255 ≈ 0.894
            Lighting.AddLight(Item.Center, 0.698f * intensity, 0.894f * intensity, 0.894f * intensity);
        }
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            player.AddBuff(ModContent.BuffType<BuffsFlyingIceBlade>(), 3600);
    ...
    public class FlyingIceBladeProj : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingIceBlade";
        NPC targetNPC = null;
        public override void SetStaticDefaults()
        {
        ...
        //{
        //    writer.Write(targetNPC != null ? targetNPC.whoAmI : -1);
        //}
        ...
        public override bool PreAI()
        {
            ...
            if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingIceBlade>()))
            {
                Projectile.timeLeft = 2;
            }
            if (!player.HasBuff(ModContent.BuffType<BuffsFlyingIceBlade>())) Projectile.Kill();
            ...
            if (n >= 0 && n < Main.npc.Length || Projectile.ai[0] != 0)
            {
                ...
                {
                    int projType = ProjectileID.IceBolt;
                    int damage = (int)(Projectile.damage / Main.rand.NextFloat(0.5f, 0.8f)); // 可根据需要调整
                    float knockback = 2f;
                    //随机值
                    float rand = Main.rand.NextFloat(0.5f, 2f);
                    Vector2 velocity = (targetNPC.Center - Projectile.Center).SafeNormalize(Vector2.UnitY) * 16f; // 朝向目标中心
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            //c.A = 0;
            // 使用自定义颜色
            Color LightsColor = new Color(175, 238, 238);
            var value = (float)(Math.Cos(Main.timeForVisualEffects * 0.04 + Projectile.ai[2] * 0.7) * 0.3f + 0.4f);
    ...
    class BuffsFlyingIceBlade : ModBuff
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<FlyingIceBladeProj>()] > 0)//检测玩家持有的弹幕数量
            {
