...
{
    class FlyingMeowmere : ModItem
    {
        private readonly Texture2D texture = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive_H/FlyingMeowmere").Value;
        private readonly Texture2D texture_ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive_H/FlyingMeowmere_").Value;
        ...
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<FlyingMeowmereProj>();
            Item.shootSpeed = 16f;
            Item.buffType = ModContent.BuffType<BuffsFlyingMeowmere>();
        ...
        public override void PostUpdate()
        {
            ...
            //•	G: 182 / 255 ≈ 0.729
            //•	B: 193 / 255 ≈ 0.757
            Lighting.AddLight(Item.Center, 1.00f * intensity, 0.729f * intensity, 0.757f * intensity);
        }
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            player.AddBuff(ModContent.BuffType<BuffsFlyingMeowmere>(), 3600);
    ...
    public class FlyingMeowmereProj : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingMeowmere";
        NPC targetNPC = null;
        ...
        public override bool PreAI()
        {
            ...
            //if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingMeowmere>()) || player.GetModPlayer<FlyingZenithPlayer>().ZenithattackspeedTimer > 0)
            //{
            //    Projectile.timeLeft = 2;
            //}
            //if (!player.HasBuff(ModContent.BuffType<BuffsFlyingMeowmere>()) && player.GetModPlayer<FlyingZenithPlayer>().ZenithattackspeedTimer < 0) Projectile.Kill();
            ...
            {
                if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingMeowmere>()))
                {
                    Projectile.timeLeft = 2;
                }
                if (!player.HasBuff(ModContent.BuffType<BuffsFlyingMeowmere>())
                    || IsDerivedFromZenith) Projectile.Kill();
            }
            //Main.NewText(Projectile.localAI[2]);
            //Main.NewText(ModContent.ProjectileType<FlyingZenithProj>());
            //修改此参数以确定攻击范围
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            targetNPC = target;
            var player = Main.player[Projectile.owner];
            player.GetModPlayer<FlyingMeowmerePlayer>().MeowmereattackspeedTimer = 120;
        }
    ...
    public class FlyingMeowmerePlayer : ModPlayer
    {
        public int MeowmereattackspeedTimer = 0;

        public override void ResetEffects()
        {
            MeowmereattackspeedTimer = Math.Max(0, MeowmereattackspeedTimer - 1);
        }

        public override void UpdateDead()
        {
            MeowmereattackspeedTimer = 0;
        }
    }
    class BuffsFlyingMeowmere : ModBuff
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<FlyingMeowmereProj>()] > 0)//检测玩家持有的弹幕数量
            {
