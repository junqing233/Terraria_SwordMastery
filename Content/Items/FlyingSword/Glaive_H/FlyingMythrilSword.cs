...
{
    class FlyingMythrilSword : ModItem
    {
        private readonly Texture2D texture = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive_H/FlyingMythrilSword").Value;
        private readonly Texture2D texture_ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive_H/FlyingMythrilSword_").Value;
        ...
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<FlyingMythrilSwordProj>();
            Item.shootSpeed = 16f;
            Item.buffType = ModContent.BuffType<BuffsFlyingMythrilSword>();
        ...
        public override bool PreDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI)
        {
            ...
            spriteBatch.Draw(texture_, drawPosition, sourceRectangle, Color.White * 0.6f, rotation, origin, scale, SpriteEffects.None, 0f);
            return false;
        }
        public override void PostUpdate()
        {
            float intensity = 0.98f; // 控制光芒强度，越小越淡
            //Color(0, 166, 147)
            //•	R: 0 / 255 ≈ 0
            //•	G: 166 / 255 ≈ 0.471
            //•	B: 147 / 255 ≈ 0.412
            Lighting.AddLight(Item.Center, 0f * intensity, 0.471f * intensity, 0.412f * intensity);
        }
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            player.AddBuff(ModContent.BuffType<BuffsFlyingMythrilSword>(), 3600);
    ...
    public class FlyingMythrilSwordProj : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingMythrilSword";
        public override void SetStaticDefaults()
        {
        ...
        public override bool PreAI()
        {
            ...
            if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingMythrilSword>()))
            {
                Projectile.timeLeft = 2;
            }
            if (!player.HasBuff(ModContent.BuffType<BuffsFlyingMythrilSword>())) Projectile.Kill();
            ...
            {
                // 如果ai[0]等于当前随机值，则发射弹幕
                int[] validAiValues = { 35, 65, 75 };
                if (IsHit && validAiValues.Contains((int)Projectile.ai[0]))
                {
                    Projectile.NewProjectile(
                        Projectile.GetSource_FromThis(),
                        Projectile.Center,
                        Vector2.Zero,
                        ModContent.ProjectileType<FlyingMythrilSwordProj_P>(),
                        (int)(Projectile.damage * Main.rand.NextFloat(1f , 1.51f)),
                        1f,
                        Projectile.owner,
                        (int)Projectile.ai[0] // 传递当前ai[0]值给新弹幕
                    );
                    IsHit = false;
                }
                return base.PreAI();
        ...
        {
            var player = Main.player[Projectile.owner];
            player.GetModPlayer<FlyingMythrilSwordPlayer>().MythrilattackspeedTimer = 120;
            IsHit = true;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            //c.A = 0;
            // 使用自定义颜色
            Color LightsColor = new Color(0, 166, 147);
            var value = (float)(Math.Cos(Main.timeForVisualEffects * 0.04 + Projectile.ai[2] * 0.7) * 0.3f + 0.4f);
    ...
    public class FlyingMythrilSwordProj_P : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingMythrilSword";
        private bool IsHit = false;
        public override void SetStaticDefaults()
    ...
    public class FlyingMythrilSwordPlayer : ModPlayer
    {
        public int MythrilattackspeedTimer = 0;
        public override void ResetEffects()
        {
            MythrilattackspeedTimer = Math.Max(0, MythrilattackspeedTimer - 1);
        }
        public override void UpdateDead()
        {
            MythrilattackspeedTimer = 0;
        }
    }
    class BuffsFlyingMythrilSword : ModBuff
    {
        public override void SetStaticDefaults()
        {
        ...
        {
            if (player.GetModPlayer<FlyingMythrilSwordPlayer>().MythrilattackspeedTimer > 0)
            {
                player.GetArmorPenetration(DamageClass.SummonMeleeSpeed) += 20; // 增加鞭子穿透率
            }
            // 如果召唤物存在，那么延长buff时间，否则删除BUFF
            if (player.ownedProjectileCounts[ModContent.ProjectileType<FlyingMythrilSwordProj>()] > 0)//检测玩家持有的弹幕数量
            {
