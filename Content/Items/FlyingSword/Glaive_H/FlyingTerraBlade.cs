...
{
    class FlyingTerraBlade : ModItem
    {
        private readonly Texture2D texture = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive_H/FlyingTerraBlade").Value;
        private readonly Texture2D texture_ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive_H/FlyingTerraBlade_").Value;
        ...
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<FlyingTerraBladeProj>();
            Item.shootSpeed = 16f;
            Item.buffType = ModContent.BuffType<BuffsFlyingTerraBlade>();
        ...
        {
            float intensity = 2.6f; // 控制光芒强度
            Color LightsColor = new Color(74, 255, 0);
            Color LightsColor_ = new Color(100, 233, 134);

            // t 在 0~1 之间波动，实现动态渐变
            float t = (float)Math.Sin(Main.GlobalTimeWrappedHourly) * 0.5f + 0.5f;
            Color mixColor = Color.Lerp(LightsColor, LightsColor_, t);

            // Lighting.AddLight 需要 0~1 的 float 值
            Lighting.AddLight(Item.Center,
                mixColor.R / 255f * intensity,
                mixColor.G / 255f * intensity,
                mixColor.B / 255f * intensity
            );
        }
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            player.AddBuff(ModContent.BuffType<BuffsFlyingTerraBlade>(), 3600);
        ...
        {        
            CreateRecipe()
               .AddIngredient(ModContent.ItemType<FlyingTrueNightsEdge>(), 1)
               .AddIngredient(ModContent.ItemType<FlyingTrueExcalibur>(), 1)
               .AddIngredient(ItemID.BrokenHeroSword, 1)
               .AddTile(ModContent.TileType<BladeForgeTile>())
               .Register();
        }
    }
    public class FlyingTerraBladeProj : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingTerraBlade";
        NPC targetNPC = null;
        ...
        public override bool PreAI()
        {
            ...
            {
                if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingTerraBlade>()))
                {
                    Projectile.timeLeft = 2;
                }
                if (!player.HasBuff(ModContent.BuffType<BuffsFlyingTerraBlade>()) 
            ...
            if (n >= 0 && n < Main.npc.Length || Projectile.ai[0] != 0)
            {
                if (targetNPC != null && targetNPC.active && (Projectile.ai[0] == 68 || Projectile.ai[0] == 1
                    || Projectile.ai[0] == 45 || Projectile.ai[0] == 30))
                {
                    //随机值
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            targetNPC = target;
            var player = Main.player[Projectile.owner];
            player.GetModPlayer<FlyingTerraBladePlayer>().TerraBladeattackspeedTimer = 120;

            float radius = 20f; // 距离目标中心的半径
            //int projType = ModContent.ProjectileType<FlyingTrueExcaliburProj_P>();
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color LightsColor = new Color(74, 255, 0);
            Color LightsColor_ = new Color(100, 233, 134);
            float t = (float)Math.Sin(Main.GlobalTimeWrappedHourly) * 0.5f + 0.5f; // t在0~1之间波动
            Color mixColor = Color.Lerp(LightsColor, LightsColor_, t);
            var value = (float)(Math.Cos(Main.timeForVisualEffects * 0.04 + Projectile.ai[2] * 0.7) * 0.3f + 0.4f);
            var v3 = Main.rgbToHsl(mixColor);
    ...
    public class FlyingTerraBladeProj_P_ : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextFloat() * 2.6f < Projectile.Opacity * 0.6f)
            {
                ...
                Color dustColor = Color.Lerp(Color.ForestGreen, Color.White, Main.rand.NextFloat() * 0.3f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color backDarkColor = new Color(50, 205, 50) * 0.8f;
            Color middleMediumColor = new Color(102, 255, 0);
            Color frontLightColor = new Color(0, 159, 107);
    ...
    public class FlyingTerraBladeProj_P : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            float endScale = 0.8f;
            float scale;
            if (percentageOfLife < 0.5f)
            {
                // 前半段：从minScale平滑放大到maxScale
                scale = MathHelper.SmoothStep(minScale, maxScale, percentageOfLife / 0.5f);
            }
            else
            {
                // 后半段：从maxScale平滑缩小到endScale
                scale = MathHelper.SmoothStep(maxScale, endScale, (percentageOfLife - 0.5f) / 0.5f);
            }
            Projectile.scale = scale * scaleFactor;
            ...
            {
                Color dustColor = Color.Lerp(Color.LightGreen, Color.White, Main.rand.NextFloat() * 0.3f);
                Dust coloredDust = Dust.NewDustPerfect(Projectile.Center + dustRotation.ToRotationVector2() * (Main.rand.NextFloat() * 80f * Projectile.scale + 20f * Projectile.scale), DustID.FireworksRGB, dustVelocity * 1f, 100, dustColor, 0.4f);
            ...
            {
                Dust dust = Dust.NewDustPerfect(dustPosition, DustID.FireworksRGB, dustVelocity, 100, Color.LawnGreen * Projectile.Opacity, Projectile.Opacity);
                dust.noGravity = true;
            }
            // 生命周期结束时销毁
            if (currentTime >= maxTime)
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.EntitySpriteDraw(texture, position, sourceRectangle, frontLightColor /* lightingColor*/ * lerpTime, Projectile.rotation - MathHelper.PiOver4 * 0.7f, origin, scale, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, sourceRectangle, frontLightColor /* lightingColor*/ * lerpTime, Projectile.rotation + MathHelper.PiOver4 * 0.2f, origin, scale, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, sourceRectangle, backDarkColor /* lightingColor*/ * lerpTime, Projectile.rotation + direction * MathHelper.PiOver4 * -1f * (1f - percentageOfLife), origin, scale, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, sourceRectangle, faintLightingColor * 0.15f, Projectile.rotation + direction * 0.01f, origin, scale, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, sourceRectangle, middleMediumColor /* lightingColor*/ * lerpTime * 0.3f, Projectile.rotation, origin, scale, spriteEffects, 0f);
            
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.White * lerpTime, Projectile.rotation + direction * 0.01f, origin, scale, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.White * 0.8f * lerpTime, Projectile.rotation + direction * -0.05f, origin, scale * 0.8f, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.White * 0.6f * lerpTime, Projectile.rotation + direction * -0.2f, origin, scale * 0.6f, spriteEffects, 0f);
            ...
            for (float i = 0f; i < 8f; i += 1f)
            {
                ...
                // 半圆分布高亮段数量
                int arcCount = 32;
                float arcAngle = MathHelper.Pi*0.88f; // 半圆
                float arcStep = arcAngle / (arcCount - 1);

                for (int j = 0; j < arcCount; j++)
                {
                    // -Pi/2 到 +Pi/2 分布
                    float offset = -arcAngle / 2 + arcStep * j;
                    float arcRotation = edgeRotation + offset;

                    DrawPrettyStarSparkle(
                        Projectile.Opacity,
                        SpriteEffects.None,
                        position + arcRotation.ToRotationVector2() * (texture.Width * 0.5f - 6f) * scale,
                        new Color(255, 255, 255, 0) /** lerpTime*/ * (i / 9f),
                        new Color(57, 255, 20)/* * (Projectile.timeLeft / 20)*/,
                        percentageOfLife,
                        0f, 0.5f, 0.8f, 1f,
                        arcRotation,
                        new Vector2(0f, Utils.Remap(percentageOfLife, 0f, 1f, 3f, 0f)) * scale,
                        Vector2.One * scale
                    );
                }
            }
            ...
            // 半圆分布高亮段数量
            int arcCount2 = 5;
            float arcAngle2 = MathHelper.Pi * 0.5f;
            float arcStep2 = arcAngle2 / (arcCount2 - 1);

            for (int k = 0; k < arcCount2; k++)
            {
                float offset2 = -arcAngle2 / 2 + arcStep2 * k;
                float arcRotation2 = Projectile.rotation + Utils.Remap(percentageOfLife, 0f, 1f, 0f, MathHelper.PiOver4) * direction + offset2;
                Vector2 drawPos2 = position + arcRotation2.ToRotationVector2() * (texture.Width * 0.5f - 4f) * scale;

                // 计算中心度，0为边缘，1为中间
                float centerFactor = 1f - Math.Abs((k - (arcCount2 - 1) / 2f) / ((arcCount2 - 1) / 2f));
                // 你可以用MathF.Pow(centerFactor, 1.5f)等方式调整分布曲线

                // 设定最大最小scale
                float minScaleX = 0.5f; // 边缘最小
                float maxScaleX = 2f; // 中间最大
                float scaleX = MathHelper.Lerp(minScaleX, maxScaleX, centerFactor);
                ...
                DrawPrettyStarSparkle(
                    ...
                    0f, 0.5f, 0.8f, 1f,
                    arcRotation2,
                    new Vector2(2f, Utils.Remap(percentageOfLife, 0f, 1f, 4f, 1f)) * scale,
                    new Vector2(scaleX, 1f) // fatness参数，X轴用scaleX
                );
            }
            return false;
        }
        ...
        {
            Texture2D sparkleTexture = TextureAssets.Extra[98].Value;
            Color bigColor = shineColor * opacity * 0.2f;
            bigColor.A = 0;
            Vector2 origin = sparkleTexture.Size() / 2f;
            float lerpValue = Utils.GetLerpValue(fadeInStart, fadeInEnd, flareCounter, clamped: true) * Utils.GetLerpValue(fadeOutEnd, fadeOutStart, flareCounter, clamped: true);
            Vector2 scaleLeftRight = new Vector2(fatness.X * 0.6f, scale.X * 0.8f) * ((lerpValue));
            Vector2 scaleUpDown = new Vector2(fatness.Y * 0.2f, scale.Y*0.6f) * lerpValue;
            bigColor *= lerpValue;
            Main.EntitySpriteDraw(sparkleTexture, drawPos, null, bigColor, MathHelper.PiOver2 + rotation, origin, scaleLeftRight, dir);
            Main.EntitySpriteDraw(sparkleTexture, drawPos, null, bigColor, MathHelper.PiOver4 + rotation, origin, scaleLeftRight, dir);
            Main.EntitySpriteDraw(sparkleTexture, drawPos, null, bigColor, MathHelper.PiOver4 + rotation + MathHelper.PiOver2, origin, scaleLeftRight, dir);
            Main.EntitySpriteDraw(sparkleTexture, drawPos, null, bigColor, 0f + rotation, origin, scaleUpDown, dir);
            Main.EntitySpriteDraw(sparkleTexture, drawPos, null, bigColor, MathHelper.PiOver2 + rotation, origin, scaleLeftRight*0.6f, dir);
            Main.EntitySpriteDraw(sparkleTexture, drawPos, null, bigColor, MathHelper.PiOver4 + rotation, origin, scaleLeftRight * 0.6f, dir);
            Main.EntitySpriteDraw(sparkleTexture, drawPos, null, bigColor, MathHelper.PiOver4 + rotation + MathHelper.PiOver2, origin, scaleLeftRight * 0.6f, dir);
            Main.EntitySpriteDraw(sparkleTexture, drawPos, null, bigColor, 0f + rotation, origin, scaleUpDown * 0.6f, dir);

        }
    }
    public class FlyingTerraBladePlayer : ModPlayer
    {
        public int TerraBladeattackspeedTimer = 0;

        public override void ResetEffects()
        {
            TerraBladeattackspeedTimer = Math.Max(0, TerraBladeattackspeedTimer - 1);
        }

        public override void UpdateDead()
        {
            TerraBladeattackspeedTimer = 0;
        }
    }
    class BuffsFlyingTerraBlade : ModBuff
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<FlyingTerraBladeProj>()] > 0)//检测玩家持有的弹幕数量
            {
