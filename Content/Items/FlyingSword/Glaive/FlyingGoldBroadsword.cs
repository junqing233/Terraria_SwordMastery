...
{
    class FlyingGoldBroadsword : ModItem
    {
        private readonly Texture2D texture = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingGoldBroadsword").Value;
        private readonly Texture2D texture_ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingGoldBroadsword_").Value;
        ...
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<FlyingGoldBroadswordProj>();
            Item.shootSpeed = 16f;
            Item.buffType = ModContent.BuffType<BuffsFlyingGoldBroadsword>();
        ...
        {
            player.AddBuff(ModContent.BuffType<BuffsFlyingGoldBroadsword>(), 3600);
    ...
    public class FlyingGoldBroadswordProj : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingGoldBroadsword";
        public override void SetStaticDefaults()
        {
        ...
        public override bool PreAI()
        {
            ...
            if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingGoldBroadsword>()))
            {
                Projectile.timeLeft = 2;
            }
            if (!player.HasBuff(ModContent.BuffType<BuffsFlyingGoldBroadsword>())) Projectile.Kill();
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            //c.A = 0;
            // 使用铁色
            Color goldColor = new Color(255, 255, 102);
            var value = (float)(Math.Cos(Main.timeForVisualEffects * 0.04 + Projectile.ai[2] * 0.7) * 0.3f + 0.4f);
            var lig = lightColor.ToVector3().Length() / 1.75f;
            var v3 = Main.rgbToHsl(goldColor);
            v3.X += ((float)Math.Cos(Main.timeForVisualEffects * 0.06 + Projectile.ai[2] * 0.5) * 0.5f + 0.5f) * 0.1f;
            var c = Main.hslToRgb(v3) * lig;
            c.A = 0;
    ...
    class BuffsFlyingGoldBroadsword : ModBuff
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<FlyingGoldBroadswordProj>()] > 0)//检测玩家持有的弹幕数量
            {
