...
{
    class FlyingBladeofGrass : ModItem
    {
        private readonly Texture2D texture = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingBladeofGrass").Value;
        private readonly Texture2D texture_ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingBladeofGrass_").Value;
        ...
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<FlyingBladeofGrassProj>();
            Item.shootSpeed = 16f;
            Item.buffType = ModContent.BuffType<BuffsFlyingBladeofGrass>();
        ...
        public override void PostUpdate()
        {
            ...
            //•	G: 12 / 255 ≈ 0.047
            //•	B: 12 / 255 ≈ 0.047
            Lighting.AddLight(Item.Center, 0.8f * intensity, 0.05f * intensity, 0.05f * intensity);
        }
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            player.AddBuff(ModContent.BuffType<BuffsFlyingBladeofGrass>(), 3600);
        ...
        public override void AddRecipes()
        {
            CreateRecipe()
                ...
                .AddIngredient(ItemID.JungleSpores, 15)// 丛林孢子
                .AddIngredient(ItemID.Vine, 3)// 藤蔓
                .AddTile(ModContent.TileType<BladeForgeTile>())
                .Register();
        }
    }
    public class FlyingBladeofGrassProj : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingBladeofGrass";
        ...
        public override bool PreAI()
        {
            ...
            {
                if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingBladeofGrass>()))
                {
                    Projectile.timeLeft = 2;
                }
                if (!player.HasBuff(ModContent.BuffType<BuffsFlyingBladeofGrass>())
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            int damage = (int)(Projectile.damage / Main.rand.NextFloat(1, 4));
            float knockback = 2f;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            //c.A = 0;
            // 使用自定义颜色
            Color LightsColor = new Color(204, 255, 0)* 0.6f;
            var value = (float)(Math.Cos(Main.timeForVisualEffects * 0.04 + Projectile.ai[2] * 0.7) * 0.3f + 0.4f);
            var v3 = Main.rgbToHsl(LightsColor);
            v3.X += ((float)Math.Cos(Main.timeForVisualEffects * 0.06 + Projectile.ai[2] * 0.5) * 0.5f + 0.5f) * 0.2f;
            var c = Main.hslToRgb(v3) /** lig*/;
            c.A = 0;
    ...
    class BuffsFlyingBladeofGrass : ModBuff
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<FlyingBladeofGrassProj>()] > 0)//检测玩家持有的弹幕数量
            {
