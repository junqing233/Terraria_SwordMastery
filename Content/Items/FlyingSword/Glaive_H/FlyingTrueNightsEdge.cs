...
{
    class FlyingTrueNightsEdge : ModItem
    {
        private readonly Texture2D texture = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive_H/FlyingTrueNightsEdge").Value;
        private readonly Texture2D texture_ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive_H/FlyingTrueNightsEdge_").Value;
        ...
        public override void SetDefaults()
        {
            ...
            Item.noMelee = true;
            Item.knockBack = 2.375f;
            Item.value = 20000;
            Item.rare = ItemRarityID.Yellow;
            Item.UseSound = SoundID.Item100;
            Item.shoot = ModContent.ProjectileType<FlyingTrueNightsEdgeProj>();
            Item.shootSpeed = 16f;
            Item.buffType = ModContent.BuffType<BuffsFlyingTrueNightsEdge>();
        ...
        {
            float intensity = 1.6f; // 控制光芒强度
            Color LightsColor = new Color(138, 43, 226);
            Color LightsColor_ = new Color(70, 203, 24);
        ...
        {
            player.AddBuff(ModContent.BuffType<BuffsFlyingTrueNightsEdge>(), 3600);
        ...
        {        
            CreateRecipe()
               .AddIngredient(ModContent.ItemType<FlyingNightsEdge>(), 1)
    ...
    public class FlyingTrueNightsEdgeProj : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingTrueNightsEdge";
        NPC targetNPC = null;
        ...
        public override bool PreAI()
        {
            ...
            {
                if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingTrueNightsEdge>()))
                {
                    Projectile.timeLeft = 2;
                }
                if (!player.HasBuff(ModContent.BuffType<BuffsFlyingTrueNightsEdge>()) 
            ...
            if (n >= 0 && n < Main.npc.Length || Projectile.ai[0] != 0)
            {
                if (targetNPC != null && targetNPC.active && (Projectile.ai[0] == 68 || Projectile.ai[0] == 1))
                {
                    ...
                    Projectile.owner,
                    rand, 1f);
                    if (proj.ModProjectile is FlyingTrueNightsEdgeProj_P customProj)
                    ...
                    var proj_ = Projectile.NewProjectileDirect(
                        ...
                        Projectile.owner,
                        rand);
                    if (proj_.ModProjectile is FlyingTrueNightsEdgeProj_P customProj_)
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            targetNPC = target;
            var player = Main.player[Projectile.owner];
            player.GetModPlayer<FlyingTrueNightsEdgePlayer>().TrueNightsEdgeattackspeedTimer = 120;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 1; i < maxStep; i++)
            {
                ...
                {
                    float factor = (1 - (float)i / ProjectileID.Sets.TrailCacheLength[Type]) * 0.8f + 0.5f;
    ...
    public class FlyingTrueNightsEdgeProj_P : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextFloat() * 2.6f < Projectile.Opacity * 0.6f)
            {
                ...
                Color dustColor = Color.Lerp(Color.Green, Color.White, Main.rand.NextFloat() * 0.3f);
            ...
            {
                Dust dust = Dust.NewDustPerfect(dustPosition, DustID.FireworksRGB, dustVelocity, 100, Color.YellowGreen * Projectile.Opacity, Projectile.Opacity);
                dust.noGravity = true;
            }
            // 视觉特效
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color backDarkColor = new Color(0, 255, 0);
            Color middleMediumColor = new Color(223, 255, 0);
            Color frontLightColor = new Color(223, 255, 0);
            ...
            Main.EntitySpriteDraw(texture, position, sourceRectangle, backDarkColor /* lightingColor*/ * lerpTime, Projectile.rotation + direction * MathHelper.PiOver4 * -1f * (1f - percentageOfLife) + (Projectile.ai[1]==1?MathHelper.Pi:0), origin, scale, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, sourceRectangle, faintLightingColor * 0.15f, Projectile.rotation + direction * 0.01f + (Projectile.ai[1] == 1 ? MathHelper.Pi : 0), origin, scale, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, sourceRectangle, middleMediumColor /* lightingColor*/ * lerpTime * 0.3f, Projectile.rotation + (Projectile.ai[1] == 1 ? MathHelper.Pi : 0), origin, scale, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, sourceRectangle, frontLightColor /* lightingColor*/ * lerpTime * 0.5f, Projectile.rotation + (Projectile.ai[1] == 1 ? MathHelper.Pi : 0), origin, scale * 0.975f, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.White * 0.6f * lerpTime, Projectile.rotation + direction * 0.01f + (Projectile.ai[1] == 1 ? MathHelper.Pi : 0), origin, scale, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.White * 0.5f * lerpTime, Projectile.rotation + direction * -0.05f + (Projectile.ai[1] == 1 ? MathHelper.Pi : 0), origin, scale * 0.8f, spriteEffects, 0f);
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.White * 0.4f * lerpTime, Projectile.rotation + direction * -0.2f + (Projectile.ai[1] == 1 ? MathHelper.Pi : 0), origin, scale * 0.6f, spriteEffects, 0f);
    ...
    public class FlyingTrueNightsEdgePlayer : ModPlayer
    {
        public int TrueNightsEdgeattackspeedTimer = 0;

        public override void ResetEffects()
        {
            TrueNightsEdgeattackspeedTimer = Math.Max(0, TrueNightsEdgeattackspeedTimer - 1);
        }

        public override void UpdateDead()
        {
            TrueNightsEdgeattackspeedTimer = 0;
        }
    }
    class BuffsFlyingTrueNightsEdge : ModBuff
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<FlyingTrueNightsEdgeProj>()] > 0)//检测玩家持有的弹幕数量
            {
