...
{
    class FlyingBeeKeeper : ModItem
    {
        private readonly Texture2D texture = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingBeeKeeper").Value;
        private readonly Texture2D texture_ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingBeeKeeper_").Value;
        ...
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<FlyingBeeKeeperProj>();
            Item.shootSpeed = 16f;
            Item.buffType = ModContent.BuffType<BuffsFlyingBeeKeeper>();
        ...
        public override void PostUpdate()
        {
            ...
            //•	G: 246 / 255 ≈ 0.96
            //•	B: 188 / 255 ≈ 0.74
            Lighting.AddLight(Item.Center, 0.74f * intensity, 0.96f * intensity, 0.74f * intensity);
        }
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            player.AddBuff(ModContent.BuffType<BuffsFlyingBeeKeeper>(), 3600);
    ...
    public class FlyingBeeKeeperProj : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingBeeKeeper";
        private int existTimer = 0; // 计时器
        public bool IsDerivedFromZenith = false; // 标记是否为天顶剑衍生
        private readonly Texture2D texture__ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingBeeKeeper__").Value;
        public override void SetStaticDefaults()
        {
        ...
        public override bool PreAI()
        {
            ...
            {
                if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingBeeKeeper>()))
                {
                    Projectile.timeLeft = 2;
                }
                if (!player.HasBuff(ModContent.BuffType<BuffsFlyingBeeKeeper>())
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (clown > 0)
                return;
            int projType = ProjectileID.Bee;
            int damage = (int)(Projectile.damage * Main.rand.NextFloat(0.4f, 0.8f));
            float knockback = 2f;

            for (int i = 0; i < 2; i++)
            {
                float radius = Main.rand.Next(40, 80);
                ...
                if (target.CanBeChasedBy(Projectile.owner))
                {
                    ...
                    Main.projectile[proj].light = 0.2f;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            //c.A = 0;
            // 使用自定义颜色
            Color LightsColor = new Color(188, 246, 188)*0.6f;
            var value = (float)(Math.Cos(Main.timeForVisualEffects * 0.04 + Projectile.ai[2] * 0.7) * 0.3f + 0.4f);
            ...
            for (int i = 1; i < maxStep; i++)
            {
                for (float j = 0; j < 1; j += 0.3f)
                {
                    ...
                    Main.EntitySpriteDraw(texture__,
                                          oldcenter + new Vector2(Projectile.velocity.X<0?-1:0,-1),
                                          rectangle,
                                          MyColor * factor,
                                          oldRo,
                                          new Vector2(texture_.Width / 2, texture_.Height / 2 / Main.projFrames[Type]),
                                          Projectile.scale * 1.4f * factor,
                                          SpriteEffects.None,
                                          0);
                }
            }
    ...
    class BuffsFlyingBeeKeeper : ModBuff
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<FlyingBeeKeeperProj>()] > 0)//检测玩家持有的弹幕数量
            {
