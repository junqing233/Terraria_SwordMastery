...
{
    class FlyingBloodButcherer : ModItem
    {
        private readonly Texture2D texture = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingBloodButcherer").Value;
        private readonly Texture2D texture_ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive/FlyingBloodButcherer_").Value;
        ...
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<FlyingBloodButchererProj>();
            Item.shootSpeed = 16f;
            Item.buffType = ModContent.BuffType<BuffsFlyingBloodButcherer>();
        ...
        {
            player.AddBuff(ModContent.BuffType<BuffsFlyingBloodButcherer>(), 3600);
    ...
    public class FlyingBloodButchererProj : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingBloodButcherer";
        ...
        public override bool PreAI()
        {
            ...
            {
                if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingBloodButcherer>()))
                {
                    Projectile.timeLeft = 2;
                }
                if (!player.HasBuff(ModContent.BuffType<BuffsFlyingBloodButcherer>())
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            int damage = (int)(Projectile.damage / 2f); // 可根据需要调整
            float knockback = 0f;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            //c.A = 0;
            // 使用自定义颜色
            Color BloodButcherer = new Color(204, 12, 12);
            var value = (float)(Math.Cos(Main.timeForVisualEffects * 0.04 + Projectile.ai[2] * 0.7) * 0.3f + 0.4f);
            var v3 = Main.rgbToHsl(BloodButcherer);
            v3.X += ((float)Math.Cos(Main.timeForVisualEffects * 0.06 + Projectile.ai[2] * 0.5) * 0.5f + 0.5f) * 0.02f;
            var c = Main.hslToRgb(v3) /** lig*/;
            c.A = 0;
    ...
    class BuffsFlyingBloodButcherer : ModBuff
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<FlyingBloodButchererProj>()] > 0)//检测玩家持有的弹幕数量
            {
