...
{
    class FlyingTheHorsemansBlade : ModItem
    {
        private readonly Texture2D texture = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive_H/FlyingTheHorsemansBlade").Value;
        private readonly Texture2D texture_ = ModContent.Request<Texture2D>("SwordMastery/Content/Items/FlyingSword/Glaive_H/FlyingTheHorsemansBlade_").Value;
        ...
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<FlyingTheHorsemansBladeProj>();
            Item.shootSpeed = 16f;
            Item.buffType = ModContent.BuffType<BuffsFlyingTheHorsemansBlade>();
        ...
        {
            player.AddBuff(ModContent.BuffType<BuffsFlyingTheHorsemansBlade>(), 3600);
        ...
        {        
            CreateRecipe()
               ...
               .AddTile(ModContent.TileType<BladeForgeTile>()) // 铸剑台
               .Register();
        }
    }
    public class FlyingTheHorsemansBladeProj : ModProjectile
    {
        public override string Texture => GetType().Namespace.Replace(".", "/") + "/FlyingTheHorsemansBlade";
        ...
        public override bool PreAI()
        {
            ...
            {
                if (!player.dead && player.HasBuff(ModContent.BuffType<BuffsFlyingTheHorsemansBlade>()))
                {
                    Projectile.timeLeft = 2;
                }
                if (!player.HasBuff(ModContent.BuffType<BuffsFlyingTheHorsemansBlade>()) 
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (clown > 0)
                return;
            var player = Main.player[Projectile.owner];
            player.GetModPlayer<FlyingTheHorsemansBladePlayer>().TheHorsemansattackspeedTimer = 120;

            float radius = 40f; // 距离目标中心的半径
            int projType = ModContent.ProjectileType<FlyingTheHorsemansBladeProj_P>();
            ...
            if (target.CanBeChasedBy(Projectile.owner))
            {
                ...
                var proj = Projectile.NewProjectileDirect(
                    ...
                    Projectile.owner,
                    rand, 1f);
                if (proj.ModProjectile is FlyingTheHorsemansBladeProj_P customProj)
                ...
                var proj_ = Projectile.NewProjectileDirect(
                    ...
                    Projectile.owner,
                    rand);
                if (proj_.ModProjectile is FlyingTheHorsemansBladeProj_P customProj_)
                ...
                var proj__ = Projectile.NewProjectileDirect(
                    ...
                    Projectile.owner,
                    rand);
                if (proj__.ModProjectile is FlyingTheHorsemansBladeProj_P customProj__)
                    customProj__.Initialize(-1f, 30f, 0.5f);
                var proj___ = Projectile.NewProjectileDirect(
                    ...
                    Projectile.owner,
                    rand);
                if (proj___.ModProjectile is FlyingTheHorsemansBladeProj_P customProj___)
                    customProj___.Initialize(1f, 30f, 0.5f);
            }
        }
    ...
    public class FlyingTheHorsemansBladeProj_P : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (Main.rand.Next(10) >= 1 || Ishit)
                return;
            Ishit = true;
            // 计算屏幕四个角落的世界坐标
            Vector2[] corners = new Vector2[4];
            corners[0] = Main.screenPosition; // 左上
            corners[1] = Main.screenPosition + new Vector2(Main.screenWidth, 0); // 右上
            corners[2] = Main.screenPosition + new Vector2(0, Main.screenHeight); // 左下
            corners[3] = Main.screenPosition + new Vector2(Main.screenWidth, Main.screenHeight); // 右下

            // 随机选择一个角落
            int cornerIndex = Main.rand.Next(4);
            Vector2 spawnPos = corners[cornerIndex];
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color backDarkColor = new Color(204, 101, 64);
            Color middleMediumColor = new Color(183, 90, 57);
            Color frontLightColor = new Color(255, 127, 80);
    ...
    public class FlyingTheHorsemansBladePlayer : ModPlayer
    {
        public int TheHorsemansattackspeedTimer = 0;

        public override void ResetEffects()
        {
            TheHorsemansattackspeedTimer = Math.Max(0, TheHorsemansattackspeedTimer - 1);
        }

        public override void UpdateDead()
        {
            TheHorsemansattackspeedTimer = 0;
        }
    }
    class BuffsFlyingTheHorsemansBlade : ModBuff
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<FlyingTheHorsemansBladeProj>()] > 0)//检测玩家持有的弹幕数量
            {
